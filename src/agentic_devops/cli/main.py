...
def secrets_migrate() -> None:
    """Migrate legacy Fernet-encrypted tokens (pre-S-1) into the secrets manager.

    One-time helper: decrypts any surviving ``token_encrypted`` columns with
    ``DEVY_ENCRYPTION_KEY`` and writes each into the manager under the row's
    ``secret_ref`` (deriving one if absent). Idempotent; dev only.
    """
    import os as _os

    from agentic_devops.config import load_settings
    from agentic_devops.db import get_pool
    from agentic_devops.proxy.secrets import build_secrets_provider, github_secret_ref, host_secret_ref

    settings = load_settings()
    secrets = build_secrets_provider(settings)
    if not secrets.writable:
        typer.echo("Refused: read-only (prod) mode — migrate into AWS SM via your IaC instead.")
        raise typer.Exit(code=1)
    key = _os.environ.get("DEVY_ENCRYPTION_KEY")
    if not key:
        typer.echo("DEVY_ENCRYPTION_KEY is not set; nothing to decrypt (already migrated?).")
        raise typer.Exit(code=1)
    ...
    with pool.connection() as conn:
        for table, id_col, name_col, ref_fn in (
            ("github_accounts", "id", "label", github_secret_ref),
            ("hosts", "id", "fqdn", host_secret_ref),
        ):
            # token_encrypted may already be dropped on fresh installs — skip if so.
            has_col = conn.execute(
                "SELECT 1 FROM information_schema.columns WHERE table_name=%s AND column_name='token_encrypted'",
                (table,),
            ).fetchone()
            if not has_col:
                continue
            rows = conn.execute(
                f"SELECT {id_col}, {name_col}, secret_ref, token_encrypted FROM {table} "
                "WHERE token_encrypted IS NOT NULL"
            ).fetchall()
            for _id, name, ref, blob in rows:
                ref = ref or ref_fn(name)
                try:
                    value = fernet.decrypt(bytes(blob)).decode("utf-8")
                except Exception as exc:  # noqa: BLE001
                    typer.echo(f"  ! {table}:{name} decrypt failed ({exc}); skipping")
                    continue
                secrets.set(ref, value)
                if not conn.execute(f"SELECT secret_ref FROM {table} WHERE {id_col}=%s", (_id,)).fetchone()[0]:
                    conn.execute(f"UPDATE {table} SET secret_ref=%s WHERE {id_col}=%s", (ref, _id))
                typer.echo(f"  ✓ {table}:{name} → {ref}")
                migrated += 1
    typer.echo(f"Migrated {migrated} secret(s). You may now drop the token_encrypted columns.")
